...
from utils.felt_paths import INPUT_DIR, OUTPUT_DIR, RAW_MOTION_DIR  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--video-root", type=Path, default=INPUT_DIR)
...
def source_reference_status(
    value: str,
    expected_stem: str,
    video_root: Path,
) -> tuple[bool, bool, bool]:
    """Return stem-match, portability, and resolvability for one source value."""
    reference = Path(value)
    stem_matches = reference.stem == expected_stem
    portable = (
        not reference.is_absolute()
        and len(reference.parts) == 2
        and reference.parts[0].startswith("Actor_")
        and reference.suffix.lower() == ".mp4"
    )
    resolved = reference if reference.is_absolute() else video_root / reference
    return stem_matches, portable, resolved.is_file()
...
def audit_extraction(
    ...
    *,
    ...
    for file_index, path in enumerate(paths, start=1):
        ...
        input_statuses = [
            source_reference_status(value, path.stem, video_root)
            for value in input_values
        ]
        input_stem_match = bool(input_values) and all(
            status[0] for status in input_statuses
        )
        input_references_portable = bool(input_values) and all(
            status[1] for status in input_statuses
        )
        input_paths_exist = bool(input_values) and all(
            status[2] for status in input_statuses
        )
        if not input_stem_match:
            issues.append("input_stem_mismatch")
        if not input_paths_exist:
            issues.append("input_path_missing")
        if not input_references_portable:
            issues.append("input_reference_nonportable")
        ...
        file_rows.append(
            {
                ...
                "input_references_portable": input_references_portable,
...
def write_reports(
    ...
    summary: dict[str, Any] = {
        ...
        "nonportable_input_reference_files": int(
            (~bool_series(files["input_references_portable"])).sum()
        ),
